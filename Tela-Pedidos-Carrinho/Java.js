// criar a viariável modalKey sera global
let modalKey = 0
...
const botoesFechar = () => {
  ...
  */
  // botoes fechar modal
  selecionaTodos('.pizzaInfo--cancelButton, .pizzaInfo--cancelMobileButton').forEach( (item) => item.
      ...
      const escolherTamanhoPreco = (key) => {
        ...
        selecionaTodos('.pizzaInfo--size').forEach((size, sizeIndex) => {
          size.addEventListener('click', (e) => {
            // clicou em um item, tirar a selecao dos outros e marca o que você clicou
            // tirar a selecao de tamanho atual e selecionar o tamanho grande
            seleciona('.pizzaInfo--size.selected').classList.remove('selected')
            // marcar o que você clicou, ao invés de usar e.target use size, pois ele é nosso item dentro
            // do loop
...
const atualizarCarrinho = () => {
  // exbir número de itens no carrinho
  ...
  if(cart.length > 0) {
    ...
    for(let i in cart) {
      ...
      // fazer o clone, exibir nas telas e depois preencher as informações
      let cartItem = seleciona('.models .cart--item').cloneNode(true)
      seleciona('.cart').append(cartItem)

      let pizzasSizeName = cart[i].size
...
const finalizarCompra = () => {
  seleciona('.cart--finalizar').addEventListener('click', () => {
    let caminho = document.querySelector('.cart--finalizar');
    console.log('Finalizar compra')
    seleciona('aside').classList.remove('show')
    seleciona('aside').style.left = '100vw'
    seleciona('header').style;display = 'flex'
    caminho.location.href = '../Tela-de-Pagamento/index.html'
  })
}
  ...
  //  console.log(item)
   ...
   preencheDadosDasPizzas(pizzaItem, item, index)
    ...
    pizzaItem.querySelector('.pizza-item a').addEventListener('click', (e) => {
        ...
        // selecionar o tamanho e preco com o clique no botão
...
var a = window.document.getElementById('continue')
  a.addEventListener('click', clicar)
  function continuar() {
  window.location.href = "../TelaHome/home.php"
}